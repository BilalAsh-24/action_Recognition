#!/usr/bin/env python
"""FIRST LOCAL MOSS GENERATION — phase-separated, bfloat16, MPS.
...
OUT_WAV = M.ROOT / "audio" / "generated" / "drinking_moss_v2_local_seed42.wav"
REPORT = M.ROOT / "results" / "moss_local_drinking_v1_generation.json"
ABORT_AVAILABLE_GB = 1.5

R: dict = {"run": "moss_local_drinking_v1", "generations": 1}
...
def main() -> int:
    ...
    seconds = round(float(M.SECONDS), 1)
    full_seconds = 30                       # max_inference_seconds; engine always denoises this
    sr = M.SAMPLE_RATE
    formatted_prompt = f"{M.PROMPT.strip()} duration: {seconds:.1f}s"
    cfg = {"model": "MOSS-SoundEffect v2.0", "device": M.DEVICE, "dtype": str(M.DTYPE),
           "prompt": M.PROMPT, "prompt_sent_to_model": formatted_prompt,
           "negative_prompt": M.NEGATIVE, "seconds": seconds,
           "denoised_seconds_internally": full_seconds,
           "num_inference_steps": M.NUM_INFERENCE_STEPS, "cfg_scale": M.CFG_SCALE,
           "sigma_shift": M.SIGMA_SHIFT, "seed": M.SEED, "denoising_strength": 1.0,
           "rand_device": "cpu", "sample_rate": sr, "num_channels": M.NUM_CHANNELS,
           "fp16_used": False, "cuda_used": False, "TORCHDYNAMO_DISABLE": "1"}
    R["config"] = cfg
    banner("MOSS-SoundEffect v2.0 — first local generation")
    ...
    with warnings.catch_warnings(record=True) as wlog:
        ...
        with torch.no_grad(), torch.autocast(M.DEVICE, dtype=M.DTYPE):
            ...
            ctx_nega = prompter.encode_prompt(M.NEGATIVE, positive=False, device=M.DEVICE)
        ...
        # ================= PHASE 2 — DIFFUSION ================================
        banner("PHASE 2 — MOSS DiT (bf16), 50 euler steps, CFG 4.0")
        ...
        pipe.scheduler.set_timesteps(M.NUM_INFERENCE_STEPS, denoising_strength=1.0,
                                     shift=M.SIGMA_SHIFT)
        ...
        latents = pipe.generate_noise(shape, seed=M.SEED, rand_device="cpu")
        ...
        R["phase2"] = {"latent_shape": list(latents.shape), "num_samples": int(num_samples),
                       "num_channels": int(num_channels), "steps": M.NUM_INFERENCE_STEPS}
        ...
        with torch.no_grad(), torch.autocast(M.DEVICE, dtype=M.DTYPE):
            for i, ts in enumerate(pipe.scheduler.timesteps):
                ...
                noise_pred = nneg.float() + M.CFG_SCALE * (npos.float() - nneg.float())
                latents = pipe.scheduler.step(noise_pred, pipe.scheduler.timesteps[i], latents)
                if (i + 1) % 10 == 0 or i == 0:
                    print(f"    step {i+1:>2}/{M.NUM_INFERENCE_STEPS}  "
